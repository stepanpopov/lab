...
//DBType-перечисление типов полей таблиц БД:
    enum DBType {
        NoType, Int32, Double, String, Date
    };
...
//===============class DBTableTxt=============================
    ...
    public:
        ...
        vector<int> IndexOfRecord(void *keyValue,
                                  string keyColumnName);// возвращает вектор из индексов data, у которых в столбце keyColumnName значение совпадает с keyValue
        string
        valueToString(Row &row, string columnName);// возвращает из Row значение в виде строки по названию столбца
